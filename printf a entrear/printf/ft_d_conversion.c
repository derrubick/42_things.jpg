...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/09/03 19:39:11 by vnastase          #+#    #+#             */
/*   Updated: 2020/09/03 19:39:15 by vnastase         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static void	ft_flags_wield(int *rst, t_flags n, int nb, int len)
{
	if (len > n.wield)
		ft_putnbr_pft(rst, nb, 1);
	...
	{
		if (ft_ifnegative_pft(rst, 1, &nb, &len))
			n.wield--;
		while (((n.wield--) - len) > 0)
...
static void	ft_flags_mix_2(int *rst, t_flags n, int nb, int len)
{
	int cpy;
	...
	if (nb >= 0)
		n.point = 0;
	ft_ifnegative_pft(rst, 1, &nb, &len);
	if (!n.lenght && !nb)
		ft_putchar_pft(rst, ' ', 1);
	else
	{
...
static void	ft_flags_mix(int *rst, t_flags n, int nb, int len)
{
	if (!n.minus)
	{
		if (nb >= 0)
...
static void	ft_flags(int *rst, t_flags n, int nb)
{
	int len;
